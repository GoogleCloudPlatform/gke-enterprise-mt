...
type TenantLister interface {
	...
	List(selector labels.Selector) (ret []*v1kubernetesapis.Tenant, err error)
...
// List lists all Tenants in the indexer.
func (s *tenantLister) List(selector labels.Selector) (ret []*v1kubernetesapis.Tenant, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1kubernetesapis.Tenant))
	})
	return ret, err
}
...
type TenantNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1kubernetesapis.Tenant, error)
...
// List lists all Tenants in the indexer for a given namespace.
func (s tenantNamespaceLister) List(selector labels.Selector) (ret []*v1kubernetesapis.Tenant, err error) {
...
// Get retrieves the Tenant from the indexer for a given namespace and name.
func (s tenantNamespaceLister) Get(name string) (*v1kubernetesapis.Tenant, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1kubernetesapis.Resource("tenant"), name)
	}
	return obj.(*v1kubernetesapis.Tenant), nil
}
